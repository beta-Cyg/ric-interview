...
func initializeDatabase(db *sql.DB, dataPath string) error {
	...
	courseColumns := `code, title, offer_dept, requirement, description,
	...
	for _, item := range source.Courses {
		...
		for _, itemSubclass := range item.Subclasses {
			...
			_, err := insertSubclass.Exec(
				...
				itemSubclass.Instructor, string(slots), item.Code, itemSubclass.IsActive,
			)
			if err != nil {
				return err
			}
	...
	log.Printf("Database initialized: %d courses", len(source.Courses))
	return nil
}
...
func coursesHandler(db *sql.DB) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		`)
		if err != nil {
			http.Error(response, "database query failed", http.StatusInternalServerError)
			return
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			if err := rows.Scan(
				...
				http.Error(response, "database result failed", http.StatusInternalServerError)
				return
			}
